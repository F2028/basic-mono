...
/* ถ้าสงสัยว่าทำไม ใช้ _ เพราะ TypeScript/JavaScript 
ขีดล่าง _ สามารถใช้เป็น numeric separator ได้ เคมั้ย ???*/
...
export class Board {
  ...
  constructor() {
    ...
    for (let position = 1; position <= this.size; position += 1) {
      ...
      this.tiles.push(new Tile(position, type, property ?? null));
    }
  }
